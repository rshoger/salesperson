...
int dist(point p1, point p2)
{
	...
	double answer = sqrt(pow(xDiff, 2.0) + pow(yDiff, 2.0));
	int realAnswer;
	answer += 0.5;
	realAnswer = static_cast<int>(answer);
...
int christofides(std::vector<point> position_vec, int numVertices, int *arr) {
	...
	for (int i = 0; i < numVertices; i++) 
	{
		...
		parentVertex[i] = 0;   //vertex has parent attribute set to NULL
	}
...
int main ()
{
	...
	nodes.push_back(p3);
	int *path = NULL;
	int length = christofides(nodes, 3, path);
